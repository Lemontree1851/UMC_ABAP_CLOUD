CLASS zcl_http_paidpay_001 DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    TYPES:
      ...
      BEGIN OF ts_response,
        ...
        _profitcenter    TYPE prctr,
        _purchasinggroup TYPE ekgrp,
...
* 1st document
        i += 1.
        lv_dr = ls_create-chargeable.
        lv_cr = -1 * ls_create-chargeable.
          ...
          VALUE #(
            ( glaccountlineitem = |001|
              ...
              _profitabilitysupplement =
                VALUE #( customer = ls_create-customer soldproduct = '5180UA160A:A62' profitcenter = ls_create-profitcenter )
            )
            ( glaccountlineitem = |002|
              glaccount = '0051000000'
              _currencyamount =
...
* 3rd document
        IF lv_fail IS INITIAL.
          i += 1.
          lv_dr = ls_create-currentstocktotal.
          lv_cr = -1 * ls_create-currentstockamount.
          lv_cr1 = -1 * ls_create-currentstocksemi.
          lv_cr2 = -1 * ls_create-currentstockfin.
          IF lv_dr <> 0
          OR lv_cr <> 0
          OR lv_cr1 <> 0
          OR lv_cr2 <> 0.
            ...
            ls_deep-%param-companycode = ls_create-companycode.
            ls_deep-%param-accountingdocumenttype = 'Z1'.
            ls_deep-%param-documentdate = ls_create-postingdate1.
            ls_deep-%param-postingdate = ls_create-postingdate1.
            ls_deep-%param-createdbyuser = sy-uname.
...
* 4th document
        IF lv_fail IS INITIAL.
          i += 1.
          lv_dr = -1 * ls_create-currentstocktotal.
          lv_cr = ls_create-currentstockamount.
          lv_cr1 = ls_create-currentstocksemi.
          lv_cr2 = ls_create-currentstockfin.
          IF lv_dr <> 0
          OR lv_cr <> 0
          OR lv_cr1 <> 0
          OR lv_cr2 <> 0.
...
* Edit output response
        ...
        ls_response-_profitcenter = ls_create-profitcenter.
        ls_response-_purchasinggroup = ls_create-purchasinggroup.
...
* Edit output response
        ...
        ls_response-_profitcenter = ls_cancel-profitcenter.
        ls_response-_purchasinggroup = ls_cancel-purchasinggroup.
        APPEND ls_response TO es_response-items.
        CLEAR: ls_response, lv_fail, i.
      ENDLOOP.
    ENDIF.
